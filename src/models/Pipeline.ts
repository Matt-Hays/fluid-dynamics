...
export default class Pipeline {
  constructor(
    ...
    if (_sections.length <= 0) throw new Error('Pipeline must have at least one section.')
    if (_startEleMin === null || _startEleMax == null)
      throw new Error('Starting elevation must be set.')
    if (_endEleMin === null || _endEleMax == null) throw new Error('Ending elevation must be set.')
    if (_startEleMin < 0 || _endEleMin < 0 || _startEleMax < 0 || _endEleMax < 0)
      throw new Error('Elevations settings must be non-negative.')
    if (_isMetric === null) throw new Error('Units must be defined.')
  }
  ...
  get pipelineLength(): number {
    return this._sections.reduce((totalLength, section) => totalLength + section.sectionLength, 0)
  }
  ...
  set kinematicViscosity(kinematicViscosity: number) {
    this._kinematicViscosity = kinematicViscosity
  }
