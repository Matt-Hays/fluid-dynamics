...
function calcRelativeRoughness(
...
function calcVelocityHead(
  ...
  return (Math.pow(finalVelocity, 2) - Math.pow(initialVelocity, 2)) / (2 * getGravityConstant(isMetric));
}
...
function calcMajorLosses(
  ...
  return calcSerghidesApproximation(
...
function calcMinorLosses(
  ...
  return kValues.reduce((acc, k) => acc + k, 0)
    * getAvgVelocityHead(flowRate, hydraulicDiameter, isMetric);
}
...
export function calcTDH(
  ...
  for (let i = 0; i < startFlowRates.length; i++) {
    const tdhWithoutStaticHead = pressureHead
      + getDayLightedVelocityHead(startFlowRates[i], endFlowRates[i], isMetric, pipeline.isEndDayLighted)
      + pipeline.sections.reduce<number>((acc: number, section: Section) =>
          acc
          + calcMajorLosses(
    ...
    headsMax.push(
      calcStaticHead(pipeline.startEleMin, pipeline.endEleMax)
      + tdhWithoutStaticHead
    )

    headsMin.push(
      calcStaticHead(pipeline.startEleMax, pipeline.endEleMin)
      + tdhWithoutStaticHead
    )
  }
...
function getAvgVelocityHead(flowRate: number, hydraulicDiameter: number, isMetric: boolean): number {
  return Math.pow(getVelocity(flowRate, hydraulicDiameter), 2) / (2 * getGravityConstant(isMetric));
}

function getDayLightedVelocityHead(startFlowRate: number, endFlowRate: number, isMetric: boolean, isDayLighted: boolean): number {
